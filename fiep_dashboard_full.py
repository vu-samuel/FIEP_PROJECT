...
try:
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from config import COMPANY_DATA_DIR
    DATA_DIR = Path(COMPANY_DATA_DIR).resolve()
...
@st.cache_data(ttl=3600, show_spinner="Loading company list...")
def get_company_files():
    """Get list of available company data files"""
    try:
        if not DATA_DIR.exists():
            raise FileNotFoundError(f"Data directory not found: {DATA_DIR}")
        return sorted([f.name for f in DATA_DIR.glob("*.csv") if f.is_file()])
    except Exception as e:
...
def load_company_data(filename):
    ...
    try:
        ...
        required_columns = {"date", "Close", "avg_sentiment"}
...
def render_main_chart(df):
    """Render the main dual-axis chart"""
    fig = Figure()
    fig.add_trace(Scatter(x=df["date"], y=df["Close"], name="Stock Price", yaxis="y1"))
    fig.add_trace(Scatter(x=df["date"], y=df["avg_sentiment"], name="Avg Sentiment", yaxis="y2"))
    fig.update_layout(
        xaxis_title="Date",
        yaxis_title="Stock Price",
        yaxis2_title="Avg Sentiment",
        yaxis2_overlaying="y",
        yaxis2_side="right",
        yaxis2_range=[-1, 1],
        height=500
...
def main():
    ...
    if not companies:
        st.error("No company data files available")
        return
    
    selected_file = st.sidebar.selectbox("Choose a company", companies)
    date_range = st.sidebar.radio("Date Range", ["All", "YTD", "1 Year", "3 Years"], index=0)
    show_zscore = st.sidebar.checkbox("Show Z-Score", True)
    alert_threshold = st.sidebar.slider("Alert Threshold", -1.0, 0.0, -0.3, 0.05)
    ...
    if show_zscore and "sentiment_zscore" in df.columns:
        st.plotly_chart(px.line(df, x="date", y="sentiment_zscore"), use_container_width=True)

    # Alerts section
    df["alert"] = df["sentiment_change"] <= alert_threshold
    if df["alert"].any():
        st.subheader("🚨 Alerts")
        st.dataframe(df[df["alert"]][["date", "avg_sentiment", "sentiment_change", "Close"]])

    # Footer
    st.markdown("---")
    st.caption("FIEP Dashboard | Powered by Streamlit")
